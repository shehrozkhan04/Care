...
if (isset($_POST['login'])) {
    ...
    if ($resultAdmin && mysqli_num_rows($resultAdmin) > 0) {
        ...
        if ($password === $user['password']) {
            ...
            echo "<div class='forgot-password'>Invalid password for admin!</div>";
        }
    }

    // Check in doctor table
    $queryEmp = "SELECT * FROM doctor WHERE email = '$email'";
    $resultEmp = mysqli_query($conn, $queryEmp);

    if ($resultEmp && mysqli_num_rows($resultEmp) > 0) {
        $user = mysqli_fetch_assoc($resultEmp);
        if ($password === $user['password']) {
            $_SESSION['employee'] = $user['email'];
            header("Location: doctor.php");
            exit;
        } else {
            echo "<div class='forgot-password'>Invalid password for employee!</div>";
        }
    }
    ...
    if ($resultCustomer && mysqli_num_rows($resultCustomer) > 0) {
        $user = mysqli_fetch_assoc($resultCustomer);
        if ($password === $user['password']) {
            $_SESSION['customer'] = $user['email'];
            header("Location: patient.php");
            exit;
        } else {
            echo "<div class='forgot-password'>Invalid password for customer!</div>";
        }
    } else {
        echo "<div class='forgot-password'>No user found in all tables!</div>";
    }
}
